...
abstract class PlaylistsService {
  ...
  Future<MusicResponse> fetchCatalogPlaylist(@Path('storefront') String storefront, @Path('id') String id);

  @GET(
    '/catalog/{storefront}/playlists/{id}/tracks?include[songs]=artists,composers&fields[artists]=name,url&extend=artistUrl,albumUrl',
  )
  Future<MusicResponse> fetchCatalogPlaylistTracks(@Path('storefront') String storefront, @Path('id') String id);
}
